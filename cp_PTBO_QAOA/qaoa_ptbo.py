...
def optimize_qaoa(N : int, G : nx, qaoa_status, answer : list) -> dict:
    ...
    #評価関数(なんと時間計測機能付き)
    def get_objective_pca (theta: np.ndarray, N : int,G: nx.graph, pca : PCA,answer : list) -> float:
        ...
        cost = PQC.cal_cost(N,G,result,answer)
        end_PQC = time.time()
        PQC_time_sum = end_PQC - start_PQC
        return -cost

    #PQCパートの時間を計測する 
    def timed_objective(betagamma,N,G,answer : list) -> float:

...
def gen_graph(num_nodes : int,edge : int ,number : int):
    
    d = {}
    
    # ランダムグラフの生成
    G = nx.erdos_renyi_graph(num_nodes,edge * 0.1)
    print(num_nodes,number)
    #d = nx.readwrite.json_graph.adjacency_data(G)
    
    file_name = "random_" + str(edge) + "/" + str(num_nodes)+ "/" + str(number) + "in.json"
    ...
    for num_nodes in range(7,21):
        for edge in range(3,10):
            for number in np.arange(0,300):
                gen_graph(num_nodes,edge,number)